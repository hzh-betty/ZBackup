...
namespace zbackup
{
    ...
    private:
        ...
        static void download(const httplib::Request &req, httplib::Response &rsp)
        {
            // 1. 根据资源路径，获取文件备份信息
            BackupInfo info;
            if (data_->getOneByURL(req.path, &info) == false)
            {
            ...
            if (info.packFlag_ == true)
            {
                ...
                if (fu.unCompress(info.realPath_) == false)
                {
        ...
        static void listshow(const httplib::Request &req, httplib::Response &rsp)
        {
            ...
            rsp.status = 200;
            logger->debug("listshow organise html file data success");
