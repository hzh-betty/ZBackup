...
namespace zbackup
{
    ...
    public:
        ...
        // 获取线程ID
        ThreadId get_id() const { return thread_->get_id(); }
    ...
    public:
        ...
        // 检查线程池是否处于运行状态
        bool check_running_state() const { return !quit_; }

        // 获取最大线程数
        size_t get_max_count() const { return max_thread_nums_.load(); }

        // 获取活动线程数
        size_t get_active_count() const { return cur_thread_nums_.load(); }

        // 获取任务队列的最大任务数
        size_t get_task_max_count() const { return task_max_size_.load(); }

        // 获取空闲线程数
        size_t get_idle_count() const { return idle_thread_nums_.load(); }

        // 获取线程池模式
        PoolMode get_pool_mode() const { return pool_mode_; }
    ...
    public:
        ...
        ~ThreadPool()
        {
            ...
            // 3. 回收执行任务所有线程
            for (auto iter = excute_threads_.begin(); iter != excute_threads_.end(); ++iter)
            {
                // 回收每个线程
                iter->second->join();
            }
    ...
    private:
        ...
        void launch_ex_thread()
        {
            ...
            excu->start(std::bind(&ThreadPool::excute_task, this));
        ...
        void launch_mo_thread()
        {
            ...
            moni->start(std::bind(&ThreadPool::monitor_idle_threads, this));
