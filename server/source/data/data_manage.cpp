...
namespace zbackup
{
    ...
    bool DataManager::get_one_by_url(const std::string &url, BackupInfo *info)
    {
        std::shared_lock<std::shared_mutex> lock(rw_mutex_);
        return storage_->get_one_by_url(url, info);
    }

    bool DataManager::get_one_by_real_path(const std::string &real_path, BackupInfo *info)
    {
        std::shared_lock<std::shared_mutex> lock(rw_mutex_);
        return storage_->get_one_by_real_path(real_path, info);
    }

    void DataManager::get_all(std::vector<BackupInfo> *arry)
    {
        std::shared_lock<std::shared_mutex> lock(rw_mutex_);
    ...
    bool DataManager::delete_one(const BackupInfo &info)
    {
        std::unique_lock<std::shared_mutex> lock(rw_mutex_);
