...
namespace zbackup
{
    ...
    bool UserManager::init_user_table() const
    {
    ...
    bool UserManager::validate_user(const std::string &username, const std::string &password) const
    {
    ...
    std::optional<UserInfo> UserManager::get_user_info(const std::string &username) const
    {
        ...
        {
            std::string sql =
                    "SELECT username, password_hash, email, created_at, is_active FROM users WHERE username = ?";
            auto result = conn->execute_query(sql, username);
    ...
    bool UserManager::update_user(const UserInfo &user_info) const
    {
        auto conn = mysql_pool_.get_connection();
        if (!conn)
        {
